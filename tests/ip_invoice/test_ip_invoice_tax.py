...
from framework.ip_invoice_tax_actions import (
    ...
    complete_master_invoice_non_taxable_order_item_and_taxable_ip_service_flow,
    complete_master_invoice_taxable_order_item_discount_and_non_taxable_ip_service_flow,
    complete_master_invoice_unlink_order_invoice_taxable_order_and_taxable_ip_service_flow,
...
def test_master_invoice_non_taxable_order_item_and_taxable_ip_service_totals_match_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Master invoice created with order with non-taxable item and IP with taxable service;
    totals match individual invoices.

    Expected:
    - IP patient is created and admitted.
    - Nursing Service is added.
    - Order request is created from IP service.
    - One non-taxable medicine item is selected from Med_3 or Med_5.
    - Sales order is confirmed.
    - Sales order invoice item Total = S.Price * QTY.
    - IP invoice Nursing Service tax = Rate * 5 / 100.
    - IP invoice Nursing Service amount = Rate + Tax.
    - Master Invoice Nursing Service tax and amount match IP invoice.
    - Master Invoice Net Total = Sales order item Total + Nursing Service Amount.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_non_taxable_order_item_and_taxable_ip_service_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
    ...
    assert result["patient_created"] is True
    assert result["admission_created"] is True
    assert result["nursing_service_added"] is True
    assert result["order_requested"] is True
    assert result["sales_order_confirmed"] is True
    assert result["sales_order_invoice_created"] is True
    assert result["sales_order_completed"] is True
    ...
    assert result["master_invoice_verification_result"]["master_invoice_correct"] is True
...
def test_master_invoice_taxable_order_item_discount_and_non_taxable_ip_service_totals_match_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Master invoice created with order with taxable item and added discount for that item
    and IP with non-taxable service; totals match individual invoices.
    ...
    - One taxable medicine item is selected from Med_1, Med_2, Med_4.
    - Sales order is confirmed.
    - Item Name, Rate and Quantity are captured from Sales Order Details.
    - On Demand Discount is applied before viewing order invoice.
    - Order invoice S.Price, GST, CESS, total tax and discounted Net Total are validated.
    - IP invoice is generated for Doc Visit service with no tax.
    - Master invoice is generated by linking order invoice + IP invoice.
    - Master invoice pharmacy tax matches order invoice tax.
    - Master invoice Net Total = discounted order invoice total + Doc Visit amount.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_taxable_order_item_discount_and_non_taxable_ip_service_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
    ...
    assert result["patient_created"] is True
    assert result["admission_created"] is True
    assert result["doc_visit_service_added"] is True
    assert result["order_requested"] is True
    assert result["sales_order_confirmed"] is True
    assert result["sales_order_invoice_created"] is True
    assert result["sales_order_completed"] is True
...
def test_unlink_order_invoice_from_master_taxable_order_item_and_taxable_ip_service_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Removing order invoice from Master that has taxable pharmacy item and taxable IP service;
    service invoice tax and total remain correct after unlink.

    Expected:
    - Nursing Service is added to IP.
    - Taxable medicine item Med_1 / Med_2 / Med_4 is added to order.
    - Sales order invoice tax and total are validated.
    - Nursing Service IP invoice tax and total are validated.
    - Master invoice is generated by linking order invoice + IP invoice.
    - Master invoice pharmacy tax and Nursing Service tax are validated.
    - Order invoice is unlinked from Master Invoice using captured Invoice No.
    - Nursing Service tax and amount remain same after unlink.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_unlink_order_invoice_taxable_order_and_taxable_ip_service_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
    ...
    assert result["ip_invoice_created"] is True
    assert result["master_invoice_created"] is True
    assert result["order_invoice_unlinked"] is True
    assert result["master_before_unlink_result"]["master_invoice_correct"] is True
    assert result["master_after_unlink_result"]["service_invoice_calculations_correct"] is True
...
def test_unlink_ip_invoice_from_master_taxable_service_and_taxable_order_item_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Removing IP invoice from Master that has taxable Nursing Service and
    order invoice also has taxable pharmacy item; order invoice tax and total
    should remain correct after unlink.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_unlink_ip_invoice_taxable_service_and_taxable_order_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
    ...
    assert result["ip_invoice_created"] is True
    assert result["master_invoice_created"] is True
    assert result["ip_invoice_unlinked"] is True
    assert result["master_before_unlink_result"]["master_invoice_correct"] is True
    assert result["master_after_unlink_result"]["order_invoice_calculations_correct"] is True
...
def test_master_invoice_non_taxable_order_item_and_taxable_ip_service_discount_totals_match_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Master invoice created with order with non-taxable item.
    IP with taxable service and added discount for that service;
    totals match individual invoices.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_non_taxable_order_item_and_taxable_ip_service_discount_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
    ...
    assert result["sales_order_completed"] is True
    assert result["ip_invoice_created"] is True
    assert result["ip_service_discount_applied"] is True
    assert result["master_invoice_created"] is True
    assert result["master_invoice_verification_result"]["master_invoice_correct"] is True
...
def test_master_invoice_remove_service_discount_after_master_creation_not_doubling_flow(
    page: Page,
    config,
) -> None:
    """
    Case:
    Master invoice created with order with non-taxable item.
    IP with taxable service and added discount for that.
    After Master Invoice creation, remove the discount.
    Amount should not double.
    """

    consumer_profile = generate_consumer_profile()

    result = complete_master_invoice_remove_service_discount_after_master_creation_not_doubling_flow(
        page=page,
        config=config,
        consumer_profile=consumer_profile,
    )
