...
@allure.title("New MR sharing with only case")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_NewMR_withonly_case(login):
    try:
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("New MR sharing with only Prescription")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_NewMR_withonly_prescription(login):
    try:
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("New MR sharing with only uploaded Prescription")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_NewMR_withonly_uploadedprescription(login):
...
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_NewMR_with_caseandprescriptions(login):
    try:
       ...
       # Get the current working directory
        ...
        wait_and_locate_click(login, By.XPATH, "(//div[@id='newcase_visit_card_cavis_552026f1-7827-4506-9dd5-ea3f0d9873d8'])[1]")
        time.sleep(2)
