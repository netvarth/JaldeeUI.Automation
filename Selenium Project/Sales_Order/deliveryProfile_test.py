...
def test_create_Online_order_With_Homedelivery(consumer_login):
    try:
        ...
        if match:
            ...
            assert 50 <= item_price <= 3000, f"Item price {item_price} is out of the allowed range (50 to 3000)."
        else:
            raise Exception("Item price not below the 3000.")
        WebDriverWait(consumer_login, 20).until(
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Create_Online_Order_without DeliveryCharge")

def test_create_Online_order_existingconsumer(consumer_login):
    try:
        time.sleep(5)
        consumer_data = create_consumer_data()

        # Locate and click on Dessert category
        Dessert = WebDriverWait(consumer_login, 20).until(
            EC.presence_of_element_located((By.XPATH, "//div[@class='category-name d-flex justify-content-between'][normalize-space()='Dessert']"))
        )
        consumer_login.execute_script("arguments[0].scrollIntoView(true);", Dessert)
        ...
        # Access cart and set item quantity
        WebDriverWait(consumer_login, 20).until(
            EC.presence_of_element_located((By.XPATH, "//span[@class='cart-count ng-star-inserted']"))
        ).click()
        time.sleep(3)

        item_qty = WebDriverWait(consumer_login, 20).until(
            EC.presence_of_element_located((By.XPATH, "//input[@id='vertical']"))
        )
        item_qty.clear()
        item_qty.send_keys(5)
        time.sleep(2)
        ...
        # Locate the item price element
        item_price_element = WebDriverWait(consumer_login, 20).until(
            EC.presence_of_element_located((By.XPATH, "//div[@class='d-flex justify-content-between total mt-2']//div[contains(text(),'₹')]"))
        )

        # Extract the text of the item price
        item_price_text = item_price_element.text.strip()
        ...
        if match:
            ...
            # Check for delivery charge visibility based on item price
            if item_price < 50 or item_price > 3000:
                # If the item price is out of range, check if delivery charge is displayed
                try:
                    delivery_charge_element = WebDriverWait(consumer_login, 10).until(
                        EC.presence_of_element_located((By.XPATH, "//div[contains(text(),'(+)₹50.00')]"))
                    )
                    assert delivery_charge_element.is_displayed(), "Delivery charge should be displayed for out-of-range prices."
                    print("Delivery charge is displayed as expected.")
                except TimeoutException:
                    print("Delivery charge is NOT displayed as expected for out-of-range prices.")
            else:
                # If the item price is within the valid range, check that delivery charge is NOT displayed
                try:
                    delivery_charge_element = WebDriverWait(consumer_login, 10).until(
                        EC.presence_of_element_located((By.XPATH, "//div[contains(text(),'(+)₹50.00')]"))
                    )
                    assert not delivery_charge_element.is_displayed(), "Delivery charge should NOT be displayed for prices within the range."
                    print("Delivery charge is NOT displayed as expected.")
                except TimeoutException:
                    print("Delivery charge is NOT displayed as expected for prices within the range.")

        else:
            raise Exception("Item price not found.")


        # Continue with payment regardless of delivery charge status
        WebDriverWait(consumer_login, 20).until(
            EC.presence_of_element_located((By.XPATH, "//button[normalize-space()='Pay']"))
        ).click()
        time.sleep(2)
