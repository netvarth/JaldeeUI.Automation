...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Test Case: Print")
@pytest.mark.parametrize('url', ["https://scale.jaldee.com/business/"])
def test_print_token_walkin(login):
    time.sleep(5)
    try:
        WebDriverWait(login, 15).until(
        EC.invisibility_of_element_located((By.CLASS_NAME, "overlay"))  # Adjust this class if needed
        ...
        # WebDriverWait(login, 10).until(
        #     EC.presence_of_element_located(
        #         (By.XPATH, "(//span[@class='p-dropdown-trigger-icon ng-star-inserted fa fa-caret-down'])[1]"))
        # ).click()

        time.sleep(2)
        WebDriverWait(login, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, "(//div[@class='option-container ng-star-inserted'][normalize-space()='Naveen service prepayment'])[2]"))
        ).click()
