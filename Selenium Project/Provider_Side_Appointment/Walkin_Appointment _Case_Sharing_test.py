...
########## NEW MR,CREATE CASE FROM BOOKINGS DETAILS PAGE PATIENT RECORD ##########
...
def test_withonly_case_NewMR_from_patient_record(login):
    try:
        ...
        wait_and_locate_click(driver, By.XPATH, "//button[@id='newcase_new_visit_btn']")
        time.sleep(2)
        ...
        wait_and_locate_click(driver, By.XPATH, "//button[@id='newcase_visit_add_section_menu_btn']")
        time.sleep(2)
        ...
        time.sleep(2)
        wait_and_send_keys(
             driver, By.XPATH, "(//textarea[@placeholder='Enter Step Notes'])[1]", "Notes for the steps")
        ...
        share_element = wait.until(
             EC.element_to_be_clickable((By.XPATH, "//button[@id='newcase_share_btn']"))
        )
        driver.execute_script("arguments[0].click();", share_element)

        time.sleep(2)
        wait_and_send_keys(
                driver, By.XPATH, "//textarea[@placeholder='Enter message description']", "New MR sharing with only case"
        )
...
########## NEW MR, CREATE CASE FROM APPOINTMENT DETAILS PAGE ##########
...
def test_withonly_case_NewMR_from_appointment_details(login):
    try:
        ...
        time.sleep(3)
        wait_and_locate_click(driver, By.XPATH, "//button[@id='btnCreateCase_BUS_bookAction']")
        ...
        wait_and_send_keys(driver, By.XPATH, "//textarea[@placeholder='Enter Treatment Notes']", "Note for the treatment")
...
def test_withonly_prescription_NewMR_from_appointment_details(login):
    try:
        ...
        # Click the Create Rx button
        time.sleep(1)
        wait_and_locate_click(login, By.XPATH, "//button[@id='newcase_visit_create_rx_btn']")

        time.sleep(1)
        wait_and_locate_click(login, By.XPATH, 
        "//div[contains(@class,'mat-mdc-select-value')][.//span[normalize-space()='Amber Gordon']]/ancestor::div[contains(@class,'mat-mdc-select-trigger')]")
...
########## NEW MR, CREATE CASE FROM PATIENT TAB ##########

@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Create Case from patient tab in new MR with only case")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_withonly_case_NewMR_from_patient_tab(login):
    try:
        driver = login
        wait= WebDriverWait(login, 30)
        ...
        time.sleep(5)
        wait_and_locate_click(driver, By.XPATH, "//div[@routerlinkactive='active-menu'][.//img[contains(@src,'menu/customers.png')]]")
        
        
        # Click on 3 dot against the patient
        time.sleep(2)
        wait_and_locate_click(driver, By.XPATH, "//img[contains(@src,'cust-Icon.svg')]/ancestor::button[contains(@class,'p-button')]")

        # Click on Create Case button from the menu
        time.sleep(2)
        wait_and_locate_click(driver, By.XPATH, "//button[@role='menuitem'][.//span[contains(normalize-space(),'Create Case')]]")
        ...
        time.sleep(2)
        wait_and_send_keys(driver, By.XPATH, "(//textarea[@placeholder='Enter Step Notes'])[1]", "Notes for the steps")
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Create case from patient tab in new MR with only prescription")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_withonly_prescription_NewMR_from_patient_tab(login):
    try:
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Create case from patient tab in new MR with only uploaded prescription")
@pytest.mark.parametrize("url, username, password", [(scale_url, main_scale, password)])
def test_withonly_uploadprescription_NewMR_from_patient_tab(login):
