...
def test_booking(login):
    ...
    time.sleep(2)
    WebDriverWait(login, 20).until(
        EC.presence_of_element_located((By.XPATH, "//mat-chip[contains(text(),'Change of Plans')]"))
    ).click()

    time.sleep(5)
    WebDriverWait(login, 20).until(
        EC.element_to_be_clickable((By.XPATH, "//span[contains(text(),'Confirm')][@class='mat-button-wrapper']"))
    ).click()

    time.sleep(3)
    cancel = WebDriverWait(login, 10).until(
        EC.presence_of_element_located((By.XPATH, "//span[contains(text(),'Cancelled')]"))
    )
    cancel.click()

    assert cancel.text == "Cancelled", f"Expected status to be 'Cancelled' but found '{cancel.text}'"
