...
def test_create_Online_order(consumer_login):
    try:
        ...
        Dessert.click()
        time.sleep(3)
        wait_and_locate_click(consumer_login, By.XPATH, "(//button[@type='button'][normalize-space()='Add'])[1]")
        time.sleep(3)
        ...
        # time.sleep(3)
        confirmpage_logo= WebDriverWait(consumer_login, 10).until(
            EC.presence_of_element_located((By.XPATH, "//img[@alt='Logo']"))
        )
        scroll_to_element(consumer_login, confirmpage_logo)
        time.sleep(2)
        confirmpage_logo.click()
        time.sleep(2)
        ...
        time.sleep(3)
        All_Items = WebDriverWait(consumer_login, 10).until(
            EC.presence_of_all_elements_located((By.XPATH, "//div[@class='item-name']"))
        )

        # Extract text from All_Items
        all_items_text = [item.text.strip() for item in All_Items if item.text.strip()]
        print(f"Name of All Items: {all_items_text}")

        # Click "View All" button
        wait_and_locate_click(consumer_login, By.XPATH, "//button[normalize-space()='View All']")

        # Wait for the new items to load
        View_Items = WebDriverWait(consumer_login, 10).until(
            EC.presence_of_all_elements_located((By.XPATH, "//div[@class='item-name']"))
        )

        # Extract text from View_Items
        view_items_text = [item.text.strip() for item in View_Items]
        print(f"Name of View Items: {view_items_text}")

        # Compare expected and actual item names
        print(f"Expected Item Names: {view_items_text}, Actual Item Names: {all_items_text}")
        assert all_items_text == view_items_text, f"Expected Item Names '{view_items_text}' but got Item Names '{all_items_text}'"
