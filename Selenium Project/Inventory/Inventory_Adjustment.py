...
@allure.title("Stock Ajustment with Batch disable Item")
...
def test_stock_adjustment(login):
    try:
        ...
        click_item = wait.until(
            EC.presence_of_element_located(
                (By.XPATH, "(//div[contains(text(),'Item_6')])[1]"))
        )

        login.execute_script("arguments[0].scrollIntoView();", click_item)
        click_item.click()
...
@pytest.mark.parametrize("url, username, password", [(scale_url, sales_order_scale, password)])
def test_stock_adjustment_2(login):
    ...
    try:
        ...
        wait.until(
            EC.presence_of_element_located(
                (By.XPATH, "(//td[@class='fw-bold']//div[@class='ng-star-inserted'])[2]"))
        ).click()
        ...
        print(f"Initial stock: {initial_stock_1}")
        ...
        print(f"Updated stock: {updated_stock_1}")

        # Calculate the expected stock after adding 10
        expected_stock_1 = initial_stock_1 + 10
    ...
    # Capture the updated stock after clicking "View"
        updated_stock_element = wait.until(
            EC.presence_of_element_located(
                (By.XPATH, "//tbody/tr[td[1]//div[contains(text(),'Item_batchdisable')]]/td[2]")
            )
        )
        updated_stock = int(updated_stock_element.text.strip())
        print(f"Updated stock: {updated_stock}")
        ...
        updated_stock_element_1 = wait.until(
            EC.presence_of_element_located(
                (By.XPATH, "//tr[td[1]//div[contains(text(),'98')]]/td[3]//div[contains(@class, 'text-nowrap')]")
            )
        )
    ...
    #     updated_stock_element_1 = wait.until(
    #         EC.presence_of_element_located(
    #             (By.XPATH, "//tbody/tr[td[1]//div[contains(text(),'items')]]/td[2]")
    #         )
    #     )
        updated_stock_1 = int(updated_stock_element_1.text.strip())
        print(f"Updated stock: {updated_stock_1}")
...
def test_stock_adjustment_3(login):
    time.sleep(3)
    wait = WebDriverWait(login, 20)
