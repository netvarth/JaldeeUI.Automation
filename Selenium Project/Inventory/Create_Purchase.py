...
def test_discount_in_percentage(login):
    try:
        ...
        for item in item_list:
            ...
            mrpprice.click()
            mrpprice.clear()
            mrpprice_random_number = str(random.randint(101, 200))
            mrpprice.send_keys(mrpprice_random_number)
            print("MRP of the item:", mrpprice_random_number)
...
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("Test Case: Create purchase with Add Item Button")
@pytest.mark.parametrize("url, username, password", [(scale_url, sales_order_scale, password)])
def test_create_purchase_1(login):
    try:
        ...
        for i in range(3):  # Get the last 5 rows
            ...
            item_name = f"//tbody/tr[{i+1}]/td[1]//input[@type='radio']" 
